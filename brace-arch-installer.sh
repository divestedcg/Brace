...
select yns in "Yes" "No" "Skip"; do
	case $yns in
		Yes )
			pacman -S --needed  irqbalance git openssh nano htop wget screen zip p7zip unrar pixz pigz lm_sensors zsh dialog crda android-udev parallel screenfetch ;
			if [ "$aurPackagesEnabled" = "true" ]; then
				yaourt -S --needed downgrade prezto-git;
			fi
			break;;
		No )
...
select yns in "Yes" "No" "Skip"; do
	case $yns in
		Yes )
			pacman -S --needed  whois mtr vdpauinfo stress iotop expac wavemon tree ;
			if [ "$aurPackagesEnabled" = "true" ]; then
...
if [ "$aurPackagesEnabled" = "false" ]; then
	...
	echo downgrade prezto-git gnome-shell-extension-volume-mixer numix-circle-icon-theme-git numix-icon-theme-git jd-gui launch4j android-apktool dex2jar jd-gui sdat2img android-studio sirikali-git gocryptfs multimc5 opsu chromium-pepper-flash chromium-widevine filebot pithos-git peek firejail-git lostfiles 
fi
